...
class KNeighborsClassifier:
    """ A KNN implementation imitating scikit-learn interface.
    Ｄependency
    ----------
    `scipy` : For performance reasons, use scipy to perform the underlying operations
        - `scipy.unique`
        - `scipy.spatial.distance.cdist`
        - `scipy.argsort`
        - `scipy.stats.mode`
    `sklearn.neighbors` ： Data structures that have not yet been implemented (TODO: implement of BallTree & KDTree)
    ...
    def predict(self, X):
        if self.algorithm == "brute":
            ...
            raise ValueError("unrecognized algorithm: ", str(self.algorithm))
        neighbors_y = scipy.stats.mode(self._fit_y[neighbors_ind], axis=1)[0].flatten()
